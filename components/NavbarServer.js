...
const NavbarServer = ({
  ...
  return (
    <nav className="bg-transparent fixed top-0 w-full z-50 h-16 md:h-24 px-6 md:px-10">
      <div
        id="nav"
        className={`flex flex-wrap items-center justify-between mx-auto px-4 py-1 md:py-4 my-2 md:my-5 w-full bg-background rounded-2xl transition-all duration-300 ${
          ...
          <ul className="flex flex-col p-1 md:p-4 lg:p-0 md:mt-4 bg-dark lg:bg-transparent lg:flex-row lg:space-x-8 xl:space-x-12 lg:mt-0 lg:border-0">
            {sections.map((section) => (
              <li key={section}>
                <Link
                  href={`#${section}`}
                  className={`block py-2 md:px-3 font-mono text-lg ${
                    path == "/projects"
